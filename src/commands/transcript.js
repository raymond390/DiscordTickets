...
module.exports = {
	name: 'transcript',
	description: 'Download een transcript',
	usage: '<ticket-id>',
	...
	async execute(client, message, args, {config, Ticket}) {
		...
		if (!ticket) {
			return message.channel.send(
				new MessageEmbed()
					...
					.setTitle('❌ **Onbekende ticket**')
					.setDescription('Kan geen gesloten ticket met dat ID vinden')
		...
		if (message.author.id !== ticket.creator && !message.member.roles.cache.has(config.staff_role)) {
			return message.channel.send(
				new MessageEmbed()
					...
					.setDescription(`U heeft geen toestemming om ticket ${id} te bekijken, aangezien het niet van u is en u geen staff bent.`)
					.setFooter(guild.name, guild.iconURL())
			);
		}
		...
		if (fs.existsSync(join(__dirname, file))) {
			embed.addField('Tekst transcript', 'Bekijk bijlage');
			res.files = [
				{
		...
		if (embed.fields.length < 1) embed.setDescription(`Er zijn geen teksttranscripties of archiefgegevens voor ticket ${id}`);
